...
import { getSubmissionByCurrentUserForProblem } from "@/modules/problems/actions"
...
const getDifficultyColor = (difficulty) => {
  ...
  useEffect(()=>{
    const fetchSubmissionHistory = async()=>{
      try {
        const resolvedParams = await params;
        const submissionHistory = await getSubmissionByCurrentUserForProblem(resolvedParams.id);
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (response.submission.status === "Accepted") {
        ...
        toast.error(response.submission.status);
      }
    } catch (error) {
  ...
  >
    ...
    Hints
                  ...
                  <TabsContent value="submissions" className="p-6 h-[420px] overflow-hidden">
